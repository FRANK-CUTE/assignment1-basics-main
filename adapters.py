...
from typing import IO, Any, BinaryIO, Optional, Callable
...
def run_transformer_block(
    ...
    mha = run_multihead_self_attention_with_rope(d_model=d_model, num_heads=num_heads, max_seq_len=max_seq_len,
                                                 theta=theta, q_proj_weight=weights["attn.q_proj.weight"],
                                                 k_proj_weight=weights["attn.k_proj.weight"],
                                                 v_proj_weight=weights["attn.v_proj.weight"],
                                                 o_proj_weight=weights["attn.output_proj.weight"],
                                                 in_features=pre_norm_x1, token_positions=token_positions)
    sub1 = mha + in_features
...
def run_get_lr_cosine_schedule(
    ...
    if it < warmup_iters:
        ...
        return min_learning_rate + (1 + math.cos((it - warmup_iters) * math.pi/(cosine_cycle_iters - warmup_iters))) * (max_learning_rate-min_learning_rate) / 2
    else:
